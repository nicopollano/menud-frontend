...
import { type CreateBusinessSchema, createBusinessSchema } from '@/modules/businesses/schemas/create-business.schema'
import { createBusiness } from '@/modules/businesses/services/businesses.service'
...
function CreateBusinessDrawer() {
  ...
  const onSubmit = async (values: CreateBusinessSchema) => {
    try {
      ...
      if (error) {
        return toast.custom(() => (
          <AlertError
            ...
            description={`No se pudo crear el restaurante ${values.name}.`}
  ...
  return (
    ...
    >
      <DrawerTrigger asChild>
        <Button size='small'>Agregar restaurante</Button>
      </DrawerTrigger>
      <DrawerContent>
        <Form {...form}>
          <form
          ...
          >
            ...
            <DrawerHeader className='gap-y-[1.875rem]'>
              <div className='grid gap-y-2.5'>
                <DrawerTitle>Nuevo restaurante</DrawerTitle>
            ...
            <Button type='submit' size='medium' disabled={isSubmitting}>
              {isSubmitting ? <LoaderIcon className='size-4 animate-spin' /> : 'Crear restaurante'}
            </Button>
          </form>
        </Form>
      </DrawerContent>
